...
private:
  ...
  const std::vector<std::shared_ptr<jtf_func_ipopt> > eqn_func_;
  zjucad::matrix::matrix<Ipopt::Number> &init_val_;

  hj::sparse::csc<Number, Index> HT_;
  vector<hj::sparse::csc<Number, Index> > eqn_HT_;
    ...
    Index& nnz_h_lag, IndexStyleEnum& index_style)
{
...

  {
    if(HT_.size(1) == 0 || HT_.size(2) == 0){
        func_->hes(&x[0], nnz,format,0,0,0);
        if(nnz > 0){
            HT_.resize(func_->dim(), func_->dim(), nnz);
            func_->hes(&x[0], nnz, format, 0, &HT_.ptr()[0], &HT_.idx()[0]);
          }
      }
    ...
    if(eqn_HT_.empty()){
        ...
        for(size_t eqi = 0; eqi < eqn_func_.size(); ++eqi){
            eqn_func_[eqi]->hes(&x[0], nnz, format, 0,0,0);
            if(nnz > 0){
                eqn_HT_[eqi].resize(n,n, nnz);
                eqn_func_[eqi]->hes(&x[0],nnz,format, 0, &eqn_HT_[eqi].ptr()[0],
                    &eqn_HT_[eqi].idx()[0]);
              }
          }
      }
    ...
    Index *jCol, Number* values)
{
  ...
  for(size_t eqi = 0; eqi < eqn_func_.size(); ++eqi){
      ...
      if(values == NULL)
        eqn_func_[eqi]->gra(&fake_x[0], g_jac_nnz_[eqi],&g[0], &idx[0]);
      else
        eqn_func_[eqi]->gra(x, g_jac_nnz_[eqi], &g[0], &idx[0]);
...
{  
  ...
  if(values != NULL){
      if(hj::sparse::nnz(HT_)){
          ...
          func_->hes(x, nnz, format, &HT_.val()[0], &HT_.ptr()[0], &HT_.idx()[0]);
          HT_.val() *= obj_factor;
        }
    ...
    ptree &pt) const
{
  ...
  {
    for(size_t i = 0; i < desc->get_eqn_constraint().size(); ++i)
      eqn_vec.push_back(desc->get_eqn_constraint()[i]);
  }

  shared_ptr<solver_use_ipopt::jtf_func_ipopt> obj = desc->get_objective();
  ...
  Ipopt::SmartPtr<Ipopt::TNLP> mynlp(
        new solver_use_ipopt(obj.get(),eqn_vec, init_node_handled));
