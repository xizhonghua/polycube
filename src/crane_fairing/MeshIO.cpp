...
namespace DDG
{
  ...
  int MeshIO::read(const size_t* face, const size_t face_num,
                   const double * node, const size_t node_num,
                   Mesh &mesh)
  {
    MeshData data;
    for(size_t pi = 0; pi < node_num; ++pi){
        data.positions.push_back(Vector(node[3*pi+0], node[3*pi+1], node[3*pi+2]));
      }
    data.indices.resize(face_num);
    for(size_t i = 0; i < face_num; ++i){
        for(size_t j = 0; j < 3; ++j)
          data.indices[i].push_back(Index(face[3*i+j],-2,-2));
      }
  ...
  void MeshIO :: write( size_t *face, double *node, const Mesh& mesh)
  // writes a mesh to a valid, open output stream out
  {
    int currentIndex = 1;
    map<VertexCIter,int> vertexIndex;
    ...
    for( VertexCIter v  = mesh.vertices.begin();
         ...
         v++ )
      {
        node[3*currentIndex-3] = v->position.x;
        node[3*currentIndex-2] = v->position.y;
        node[3*currentIndex-1] = v->position.z;
    ...
    for( size_t i = 0; i < mesh.faces.size(); i++ )
      {
        ...
        do
          {
            face[3*i+j] = vertexIndex[ he->vertex ]-1;
            he = he->next;
            j++;
          }
        while( he != f.he );
