...
namespace DDG
{
   ...
   int Mesh::read( const size_t* face, const size_t face_num,
                   const double * node, const size_t node_num )
   {
      int rval;
      if( !( rval = MeshIO::read( face, face_num, node, node_num, *this)))
      {
         indexElements();
         normalize();
      }
   ...
   int Mesh::write(size_t* face, double * node)const
   {
     assert(face && node);

     MeshIO::write( face, node , *this );
