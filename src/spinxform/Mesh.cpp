...
void Mesh::setCurvatureChange(const vector<double> & v_curvature, double scale)
{
  for( size_t i = 0; i < faces.size(); i++ )
  {
     ...
     for( int j = 0; j < 3; j++ )
     {
        rho[i] += v_curvature[faces[i].vertex[j]]/ 3.;
     }
  ...
  const double min_e = *min_element(rho.begin(), rho.end());
  const double max_e = *max_element(rho.begin(), rho.end());

  for(size_t i = 0; i < rho.size(); ++i){
        rho[i] = 2*scale*(rho[i]-min_e)/(max_e-min_e)-scale;
    }
  {
    vector<size_t> faces_(3*faces.size());
    vector<double> node(3 * vertices.size());
    for(size_t i = 0 ; i < faces.size(); ++i)
      for(size_t j = 0; j < 3; ++j)
      faces_[3*i+j] = faces[i].vertex[j];

    for(size_t i = 0;  i< vertices.size(); ++i)
      for(size_t j = 1; j < 4; ++j)
        node[3*i+j-1] = vertices[i][j];
    ofstream ofs("temp_face.vtk");
    tri2vtk(ofs, &node[0], node.size()/3,&faces_[0], faces_.size()/3);
    cell_data(ofs, &rho[0] , rho.size(), "rho");
